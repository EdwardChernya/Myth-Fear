...
if (!is_area_within_bounds(grid_x, grid_y, grid_x, grid_y)) {
	instance_destroy();
	exit;
}
...
destroy_square_area_grid(grid_x, grid_y, grid_x, grid_y);

var asset = new static_asset(_x, _y, grid_x, grid_y, "barrel");
...
var destroy_func = function(_self) {
	with (_self) {
		...
		var destroy_asset = new static_asset(x, y, grid_x, grid_y, "broken barrel");
		destroy_asset.sprite_index = dungeon_barrel_broken;
